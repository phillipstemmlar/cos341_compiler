...
public class SemanticAnalyzer {
	...
	public Variable.Type checkType(SyntaxNode node, Scope scope){
		if(node.type != Variable.Type.none) return node.type;

		if(node.token == Token.eToken.tok_string_literal || node.token == Token.eToken.tok_integer_literal
			...
			CompositeSyntaxNode PROC = (CompositeSyntaxNode)node;
			LeafSyntaxNode proc  = (LeafSyntaxNode)PROC.children.get(0);
			LeafSyntaxNode PROC_name  = (LeafSyntaxNode)PROC.children.get(1);
			CompositeSyntaxNode prog  = (CompositeSyntaxNode)PROC.children.get(2);
			...
			LeafSyntaxNode TYPE_val  = (LeafSyntaxNode)TYPE.children.get(0);
			...
			LeafSyntaxNode NAME_val  = (LeafSyntaxNode)NAME.children.get(0);		
			Variable var = scope.findVariableByName(NAME_val.val());
			...
			LeafSyntaxNode io = (LeafSyntaxNode)IO.children.get(0);
			CompositeSyntaxNode VAR = (CompositeSyntaxNode)IO.children.get(1);	
			...
			if(VAR.type != Variable.Type.string && VAR.type != Variable.Type.num && VAR.type != Variable.Type.bool){
				Helper.error(type_PREFIX, VAR.line(), VAR.col(), errorInvalidIOtype(io.token,VAR.type));
				ERRORS = true;
			}
			return Variable.notype();
			...
			CompositeSyntaxNode VAR = (CompositeSyntaxNode)node;
			LeafSyntaxNode VAR_val  = (LeafSyntaxNode)VAR.children.get(0);
			Variable var = scope.findVariableByName(VAR_val.val());
			Variable.Type type = (var == null)? Variable.undefined() : var.type;
			VAR_val.type = type;
			return type;
		}
			...
			CompositeSyntaxNode CALL = (CompositeSyntaxNode)node;
			LeafSyntaxNode PROC_name  = (LeafSyntaxNode)CALL.children.get(0);
			Procedure proc = scope.findProcedureByName(PROC_name.val());
